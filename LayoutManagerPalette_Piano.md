...
Il tipo AutoCAD `Autodesk.AutoCAD.DatabaseServices.LayoutManager` verrà usato costantemente. Il progetto **non deve chiamarsi/namespacciarsi `LayoutManager`**, altrimenti ogni riferimento non qualificato a `LayoutManager.Current` risolve nel namespace del progetto invece che nel tipo Autodesk (errore di compilazione). Nome progetto/namespace: **`LayoutManagerPalette`**; "Gestione Layout" resta solo la label visibile all'utente.

## Struttura progetto (nuovo, da zero)

```
LayoutManagerPalette\
    LayoutManagerPalette.sln
    LayoutManagerPalette\
        LayoutManagerPalette.csproj
...
`LayoutManagerPalette.csproj`: progetto Class Library .NET Framework 4.8, `PlatformTarget=x64`, `OutputType=Library`. Riferimenti (`HintPath`, `CopyLocal=false`) a:
